...
- **A fresh recording enters `CALIBRATING` first.** For at most 60 s the barometer solves its baseline against GNSS altitudes — as soon as eight good fixes have accumulated the baseline is solved and the phase ends early, so a strong signal costs seconds, not a minute; no trackpoints are written and no clock runs. When the phase ends the UI reports the outcome — baseline solved, or the 60 s window elapsed with the standard baseline in use — and the user taps *Start recording*. The start-anyway check runs a few seconds after the window so the baseline-solving fix can land first. Barometer-less devices skip the phase; resuming an interrupted track starts recording immediately.
...
**Calibration.** Happens during the `CALIBRATING` phase, before recording starts: collect GNSS altitudes with accuracy better than 15 m; as soon as eight good fixes have accumulated — or the 60 s window closes, whichever comes first — take the median and solve for `p₀` so the barometric altitude matches. The early exit means a strong fix is not wasted on the full window; the median and the window backstop protect a weak or jittery signal. If no usable GNSS altitude arrives, fall back to 1013.25 hPa and set `tracks.calibrated = 0` — recording then starts anyway, and the UI reports whether the baseline was solved or the window elapsed.
...
Library track cards have a Map action that opens the Map tab with that finished track loaded. The polyline renders on the basemap through the same track layer, and the camera fits to the track bounds once (a single-point track zooms to the point). The position dot is hidden and there is no follow mode for a finished track; a live recording always takes precedence, and tapping the Map tab or starting a recording clears the viewed track and returns to the live map. The track list never instantiates a map view, so this adds no GL surface beyond the Map tab's existing one and leaves the 15–25 %/h envelope unchanged.
