...
- **Distance, elevation gain and moving time accumulate only in `RECORDING`.** Elapsed time accumulates in all non-idle states.
...
The persistent notification carries **Pause/Resume**, **Waypoint** and **Stop**. The waypoint action matters: marking a spring or a junction must never require unlocking the phone.
...
**Calibration.** For the first 60 s, collect GNSS altitudes with accuracy better than 15 m, take the median, and solve for `p₀` so the barometric altitude matches. If no usable GNSS altitude arrives, fall back to 1013.25 hPa and set `tracks.calibrated = 0`.
